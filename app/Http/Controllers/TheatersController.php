...
class TheatersController extends Controller {
    ...
    protected function bill(Request $request) {
        ...
        foreach ($seat_list as $seat) {
            $exist = DB::select('SELECT id FROM tickets WHERE schedule_id = ? AND chair_num = ?', [$schedule_id, $seat]);
            if (!$exist) {
                DB::insert('INSERT INTO tickets(schedule_id, user_id, chair_num) VALUES (?,?,?)', [$schedule_id, Auth::id(), $seat]);
            }
        }
    }
    ...
    protected function adminAll() {
        ...
        return view('theaters.adminAllTheater', [
            'theaters' => $theaters
        ]);
    }

    protected function adminInfo($theater_id) {
    ...
    protected function adminDelete($theater_id) {
        DB::update('UPDATE theaters '
                . 'SET status = 0'
                . 'WHERE id = ?', [$theater_id]);
        return redirect('/admin/theaters/all');
    ...
    protected function adminAddTheater() {
        return view('theaters.adminAddTheater');
    }

    protected function adminAdd(Request $request) {
    ...
    protected function adminDetail() {
    ...
    protected function adminScheduleAll() {
        ...
        return view('theaters.adminAllSchedule', [
            'schedules' => $schedules,
            'theater_name' => $theater_name
        ]);
    }

    protected function adminAddSchedule() {
        ...
        return view('theaters.adminAddSchedule', [
            'theaters' => $theaters,
            'movies' => $movies
        ]);
    }
    ...
    protected function adminAddSche(Request $request) {
        DB::insert('INSERT INTO schedules (movie_id, theater_id, type, show_time, show_date, price, status) 
                VALUES (?,?,?,?,?,?,?)', [$request->title, $request->name, $request->type, $request->show_time,
            $request->show_date, $request->price, 1]);
    ...
    protected function adminScheduleInfo($schedule_id) {
    ...
    protected function adminUpdateSchedule(Request $request) {
        DB::update('UPDATE schedules '
