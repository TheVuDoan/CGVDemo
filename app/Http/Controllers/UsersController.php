...
class UsersController extends Controller {
    ...
    protected function ticketDelete(Request $request) {
        ...
        foreach ($seats as $seat) {
            DB::delete('DELETE FROM tickets WHERE schedule_id = ? AND chair_num = ? AND user_id = ?', [$schedule_id, $seat->chair_num, Auth::id()]);
        }
    }

        protected function adminShow() {
        $users = DB::select('WITH ticketnum AS
                            (SELECT tickets.user_id, schedules.id, count(tickets.id) AS num
                            FROM tickets INNER JOIN schedules
                            ON tickets.schedule_id = schedules.id
                            GROUP BY tickets.user_id, schedules.id)
                    SELECT users.*, sum(ticketnum.num * price) AS totalamount
                    FROM users 
                    LEFT JOIN ticketnum ON users.id = ticketnum.user_id
                    LEFT JOIN schedules ON schedules.id = ticketnum.id
